...
def generate_base_config_template():
    ...
    template = {
        ...
        "_update_log": [
            f"{datetime.now().strftime('%Y-%m-%d')}: v3.0 初始配置"
        ],
    ...
    try:
        ...
        for line in resp.text.strip().split('\n'):
            ...
            if results[code]['last_close'] > 0:
                results[code]['change_pct'] = (results[code]['price'] / results[code]['last_close'] - 1) * 100
            else:
...
def calc_tech_position_score(price: float, ma20: float, rsi: float, vol_ratio: float) -> int:
    score = 0
    if ma20 > 0:
        price_ma20_ratio = price / ma20
        if price_ma20_ratio > 1.05: score += 3
        elif price_ma20_ratio > 1.0: score += 2
        elif price_ma20_ratio > 0.95: score += 1
    
    if 50 <= rsi <= 70: score += 3
    elif 40 <= rsi < 50: score += 2
    elif rsi > 70: score += 1
...
def calc_relative_strength_score(etf_change: float, index_change: float) -> int:
    ...
    if relative_change > 1.0: score += 7
    elif relative_change > 0.5: score += 5
    elif relative_change > 0: score += 3
    elif relative_change > -0.5: score += 1
    return score
...
def refresh_etf_pool(base_scores: Dict, index_change: float):
    ...
    for code, info in ETF_WATCHLIST_BASE.items():
        ...
        vol_ma5 = last['vol_ma5'] if pd.notna(last['vol_ma5']) and last['vol_ma5'] > 0 else 1
        vol_ratio = rt['volume'] * calc_volume_time_factor() / vol_ma5 if vol_ma5 > 0 else 1.0
...
def auto_wave_type(user_input: str, buy_reason: str, holding_days: int) -> str:
    if user_input and user_input in ['快速波段', '标准波段']: return user_input
    if holding_days <= 3: return "快速波段"
    fast_signals = ["突破", "放量", "强势", "RSI<30"]
    if any(s in buy_reason for s in fast_signals): return "快速波段"
    return "标准波段"


def auto_infer_holding_info(h: Dict, history: Optional[pd.DataFrame], holding_days: int) -> Dict:
    symbol = h['symbol']
    ...
    if history is not None and len(history) >= holding_days:
        try:
            ...
            if 'date' in history_with_date.columns:
                ...
                buy_day_data = history_with_date[history_with_date['date'] <= buy_date].tail(3)
            else:
                buy_day_data = history.tail(holding_days + 3).head(3)
            
            if len(buy_day_data) > 0:
                last_day = buy_day_data.iloc[-1]
                vol_ma5 = last_day['vol_ma5'] if pd.notna(last_day['vol_ma5']) and last_day['vol_ma5'] > 0 else 1
                vol_ratio = last_day['volume'] / vol_ma5 if vol_ma5 > 0 else 1.0
                rsi = calculate_rsi(buy_day_data.tail(14) if len(history) >= 14 else buy_day_data, 14)
                ma20 = last_day['ma20'] if pd.notna(last_day['ma20']) else last_day['close']
                price_vs_ma20 = ((last_day['close'] - ma20) / ma20 * 100) if ma20 > 0 else 0
                
                reasons = []
                if vol_ratio < 0.7: reasons.append(f"缩量({vol_ratio:.2f})"); buy_score += 5
                if rsi < 50: reasons.append(f"RSI{rsi:.0f}"); buy_score += 3
                if price_vs_ma20 < 0: reasons.append("回踩MA20"); buy_score += 3
                elif price_vs_ma20 > 2: reasons.append("突破MA20"); buy_score += 5
                if vol_ratio > 1.5: reasons.append(f"放量({vol_ratio:.2f})"); buy_score += 5
                
                if reasons: buy_reason = "+".join(reasons)
                else: buy_reason = "技术买点"
        except Exception:
            pass
    
    wave_type = auto_wave_type(h.get('wave_type', ''), buy_reason, holding_days)
    return {'name': name, 'wave_type': wave_type, 'buy_score': min(buy_score, 90), 'buy_reason': buy_reason}
...
def identify_wave_status(holding_days: int, wave_type: str) -> Tuple[str, str, str]:
    ...
    if holding_days == 0:
        ...
        return ("洗盘观察期", "🟢", f"第{holding_days}天，给主力3-5天洗盘空间，坚定持有")
    elif holding_days < max_days:
        return ("价值持有期", "🟢", f"第{holding_days}天，距T+21时间止损还有{days_left}天")
    else:
...
def calculate_dynamic_stop(cost: float, current: float, peak: float) -> float:
    ...
    if gain_pct > DYNAMIC_STOP_TRIGGER:
        ...
        dynamic_stop = cost * (1 + HARD_STOP_LOSS / 100 + moves * DYNAMIC_STOP_MOVE / 100)
        return max(dynamic_stop, cost)
    else:
        return cost * (1 + HARD_STOP_LOSS / 100)


def generate_wave_action(h: Dict, price: float, profit_pct: float,
                         holding_days: int, phase: str, stop_loss: float,
                         policy_score: int = None, max_drawdown: float = 0.0) -> str:
    ...
    if policy_score is not None and policy_score < POLICY_LOGIC_STOP_THRESHOLD:
        return f"🔴 逻辑止损（板块政策分{policy_score}<15，政策逻辑证伪，尾盘14:55清仓）"
    ...
    if is_reduced:
        if max_drawdown <= -3.0:
            ...
            return (f"✅ 格局剩余仓位（浮盈 {profit_pct:.2f}%，距高点回撤 {max_drawdown:.2f}%）"
                    f" | 纪律：回撤达-3% 或 浮盈达+12% 全清")
    elif profit_pct >= 8.0:
        return (f"🎯 首次减仓（浮盈{profit_pct:.2f}%≥8%，强制触发：卖出50%锁定利润）"
                f" | 减仓后请在holdings.json中添加 \"is_reduced\": true")
    elif profit_pct >= 5.0:
        return (f"✅ 观察止盈（浮盈{profit_pct:.2f}%，接近首次减仓线+8%）"
                f" | 距-8%止损线还有{space_to_stop:.1f}% | 持仓第{holding_days}天/距T+21还有{days_left}天")

    # 洗盘期特殊提示
    if holding_days < 5 and profit_pct < 0:
        return (f"🟢 洗盘持有（浮亏{profit_pct:.2f}%，ETF价值托底，给主力{5 - holding_days}天洗盘空间）"
                f" | 距-8%硬止损线还有{space_to_stop:.1f}% | 政策分{'正常' if policy_score is None or policy_score >= 15 else f'⚠️{policy_score}接近警戒'}")

    # 常规持有
    policy_status = "正常" if policy_score is None or policy_score >= 20 else (
        f"⚠️{policy_score}接近警戒线(15)" if policy_score >= 15 else f"🔴{policy_score}触发止损"
    )
...
def scan_market():
    ...
    if advance_ratio > 60: temperature = "🔥普涨"
    elif advance_ratio > 40: temperature = "😐震荡"
    else: temperature = "❄️普跌"
    
    return {
        'scan_time': scan_time.strftime('%Y-%m-%d %H:%M'),
        'index': index, 'rising': rising, 'total': total,
        'advance_ratio': advance_ratio, 'temperature': temperature,
        'realtime': realtime
    }


def scan_holdings_with_wave_management(holdings_config: Dict, realtime: Dict, etf_pool: Dict = None):
    print("⏳ 正在扫描持仓波段状态...")
    ...
    for h in holdings_list:
        if h.get('symbol', '').startswith('_'): continue
        if h.get('qty', 0) == 0: continue
        
        code = h['symbol']
        price = realtime[code]['price'] if code in realtime else fetch_sina_realtime([code]).get(code, {}).get('price', 0)
        if price == 0: continue
        ...
        action = generate_wave_action(h, price, profit_pct, holding_days, phase, stop_loss, policy_score, max_drawdown)
        
        hard_stop_price = h['cost'] * (1 + HARD_STOP_LOSS / 100)
        space_to_hard_stop = ((price - hard_stop_price) / price * 100) if price > 0 else 0
        days_left = WAVE_CONFIG.get(h['wave_type'], WAVE_CONFIG['快速波段'])['max_days'] - holding_days

        holdings_data.append({
            'symbol': code.replace('sh', '').replace('sz', ''), 'name': h['name'],
            'qty': h['qty'], 'cost': h['cost'], 'price': price, 'profit_pct': profit_pct,
            'value': market_value, 'days': holding_days, 'wave_type': h['wave_type'],
            'phase': phase, 'emoji': emoji, 'stop': stop_loss,
            'hard_stop_price': hard_stop_price, 'space_to_hard_stop': space_to_hard_stop,
            'days_left': days_left, 'policy_score': policy_score, 'max_dd': max_drawdown, 'action': action
        })
        
        wave_cards.append({
            'name': h['name'], 'wave_type': h['wave_type'], 'emoji': emoji, 'phase_desc': phase_desc,
            'profit_pct': profit_pct, 'buy_score': h['buy_score'], 'buy_reason': h['buy_reason'],
            'action': action, 'stop_loss': stop_loss, 'hard_stop_price': hard_stop_price,
            'space_to_hard_stop': space_to_hard_stop, 'holding_days': holding_days, 'days_left': days_left,
            'max_days': WAVE_CONFIG.get(h['wave_type'], WAVE_CONFIG['快速波段'])['max_days'], 'policy_score': policy_score,
        })
        print(".", end="", flush=True)
...
def calc_volume_time_factor() -> float:
    ...
    total_minutes = 240
    if h < 9 or (h == 9 and m < 30): elapsed = total_minutes
    elif h < 11 or (h == 11 and m <= 30): elapsed = (h - 9) * 60 + m - 30
    elif h < 13: elapsed = 120
    elif h < 15: elapsed = 120 + (h - 13) * 60 + m
    else: elapsed = total_minutes
...
def scan_etf_pool(etf_pool: Dict, holding_symbols: set, realtime: Dict):
    ...
    for code, pool_info in etf_pool.items():
        ...
        if rt['change_pct'] > 1.0 and vol_ratio > 1.2: fund_flow = "💰流入"
        elif rt['change_pct'] < -1.0 and vol_ratio > 1.2: fund_flow = "💸流出"
        else: fund_flow = "➖平衡"
        
        position = "✅持仓" if code in holding_symbols else "⭕无"
        
        etf_list.append({
            'symbol': code.replace('sh', '').replace('sz', ''), 'name': pool_info['name'],
            'category': pool_info['category'], 'policy': pool_info['policy'], 'price': rt['price'],
            'change_pct': rt['change_pct'], 'vol_ratio': vol_ratio, 'rsi': rsi, 'ma20': ma20,
            'fund_flow': fund_flow, 'position': position
        })
        print(".", end="", flush=True)
...
def generate_report_v2(market, etf_list, holdings_data, wave_cards, total_value, cash_available):
    """生成 v3.0 格式报告"""
    report = []
    ...
    if h < 9 or (h == 9 and m < 30):
        time_tag, time_warn = "⚠️ 开盘前数据", "当前为开盘前，量比无意义。请勿基于此报告做任何买卖决策。"
    elif h < 11 or (h == 11 and m <= 30):
        elapsed = (h - 9) * 60 + m - 30
        time_tag, time_warn = f"🔶 上午盘中数据（{h:02d}:{m:02d}）", f"量比已折算（系数×{round(240/max(elapsed,1),2)}）。本报告为参考，不可用于尾盘决策。"
    elif h < 13:
        time_tag, time_warn = f"🔶 午休数据（{h:02d}:{m:02d}）", "午休期间量比已折算。本报告为参考，不可用于尾盘决策。"
    elif h == 14 and m < 50:
        elapsed = 120 + (h - 13) * 60 + m
        time_tag, time_warn = f"🔷 下午盘中数据（{h:02d}:{m:02d}）", f"量比已折算（系数×{round(240/max(elapsed,1),2)}）。请在14:55重新运行。"
    elif h == 14 and m >= 50:
        time_tag, time_warn = f"✅ 尾盘数据（{h:02d}:{m:02d}）", "量比为全天真实值。**本报告可直接用于金牌判断和14:55-15:00执行决策。**"
    else:
        time_tag, time_warn = f"📋 收盘后数据（{h:02d}:{m:02d}）", "收盘后数据，可用于复盘分析，不可用于当日交易。"
    ...
    if wave_cards:
        for card in wave_cards:
            profit_emoji = "🟢" if card['profit_pct'] > 0 else "🔴" if card['profit_pct'] < -3 else "🟡"
            ps = card.get('policy_score')
            if ps is None: policy_display = "未获取"
            elif ps < POLICY_LOGIC_STOP_THRESHOLD: policy_display = f"🔴 {ps}分（已触发逻辑止损）"
            elif ps < 20: policy_display = f"🟡 {ps}分（接近警戒线15）"
            else: policy_display = f"🟢 {ps}分（安全）"
            ...
            report.append(f"**买入信息:** ⭐{card['buy_score']}分 | 📝 {card['buy_reason']}")
            report.append(f"**风控参数:** 🛑 硬止损价 {card['hard_stop_price']:.3f}（距-8%线还有{card['space_to_hard_stop']:.1f}%）| ⏳ 第{card['holding_days']}天/距T+21还有{card['days_left']}天")
    ...
    if holdings_data:
        for h in holdings_data:
            ...
            report.append(f"**{h['symbol']} {h['name']}:** {profit_emoji} 现价{h['price']:.3f} | 盈亏{h['profit_pct']:+.2f}% | ⏳ 持仓{h['days']}天 | 🏷️ {h['wave_type']} | {h['emoji']} {h['phase']}\n")
    else:
        report.append("- 💤 空仓\n")
    
    report.append("\n## 📊 扫描数据（原始数据）\n")
    report.append("| 代码 | 名称 | 现价 | 涨跌% | 量比 | RSI | MA20 | 资金流向 | 政策分 | 持仓 |")
    report.append("|------|------|------|-------|------|-----|------|----------|--------|------|")
    for etf in etf_list:
        change_emoji = "🔴" if etf['change_pct'] < 0 else "🟢"
        report.append(f"| {etf['symbol']} | {etf['name']} | {etf['price']:.3f} | {change_emoji}{etf['change_pct']:+.2f}% | {etf['vol_ratio']:.2f} | {etf['rsi']:.0f} | {etf['ma20']:.3f} | {etf['fund_flow']} | {etf['policy']} | {etf['position']} |")
    
    report.append("\n## 🌡️ 市场环境\n")
    report.append(f"**🕐 扫描时间:** {market['scan_time']}")
    report.append(f"**🌡️ 市场温度:** {market['temperature']} (上涨{market['rising']}/{market['total']})")
    if market['index'].get('ok'):
        index_emoji = "📈" if market['index']['change_pct'] > 0 else "📉"
        report.append(f"**🏛️ 沪深300:** {index_emoji} {market['index']['price']:.2f} ({market['index']['change_pct']:+.2f}%)\n")
...
def main(force_refresh=False):
    ...
    try:
        ...
        holding_symbols = {h['symbol'] for h in holdings_config.get('holdings', []) if h.get('qty', 0) > 0}
        ...
        holdings_data, wave_cards, total_value = scan_holdings_with_wave_management(holdings_config, market['realtime'], etf_pool)
