...
class ObserveTests(unittest.TestCase):
    ...
    def test_rebuild_history_uses_buy_date_before_first_snapshot(self):
        ...
        index_cache = {
            "H00300": {"2026-05-21": 7000.0, "2026-05-25": 0},
            "H00905": {"2026-05-21": 10000.0, "2026-05-25": 0},
        }
        ...
        self.assertEqual(snapshots[1]["benchmarks"]["hs300_tr"]["close"], 7000.0)
        self.assertEqual(snapshots[1]["benchmarks"]["hs300_tr"]["source"], "carry_forward")
        self.assertEqual(snapshots[1]["benchmarks"]["csi500_tr"]["close"], 10000.0)
        self.assertEqual(snapshots[1]["benchmarks"]["csi500_tr"]["source"], "carry_forward")
        self.assertEqual(len(stats["series"]["hs300"]), len(snapshots))
        self.assertEqual(len(stats["series"]["csi500"]), len(snapshots))
        self.assertEqual(len(stats["series"]["enhanced_ref"]), len(snapshots))
        self.assertEqual(len(trades), 2)
        self.assertEqual(trades[1]["action"], "SELL")
        self.assertNotEqual(trades[1]["pnl_amount"], 0)
        ...
        self.assertIn("sell_price_inferred_from_cash_delta", stats["data_quality"]["notes"])
        self.assertIn("hs300_tr_carry_forward_history:2026-05-25", stats["data_quality"]["notes"])
        self.assertIn("csi500_tr_carry_forward_history:2026-05-25", stats["data_quality"]["notes"])
