...
- **系统已全部线上运行**（GitHub Actions + Gist + Cloudflare Worker），本地不再跑扫描。
  本目录是落后的镜像副本，`etf_pool.json` / `holdings.json` 均为历史快照，
  **不可据此判断当前持仓或分数**；实时状态只在 Gist。
- 唯一仍需本地手动维护的是 `etf_base_config.json`（改分后 push 生效），评分方法见 `etf_base_config/GEMINI_UPDATE_GUIDE.md`（供 Gemini 使用，Claude 不主动改分）。
- `ds_scanner.py` 依赖新浪行情 + AKShare，Cowork 沙箱不要尝试抓行情，只能对导出副本做离线分析。
...
| `scan.yml`             | Actions 工作流（仓库内位于 `.github/workflows/`）            | 手动迭代                 |
| `run_report.bat`       | 本地手动跑扫描的批处理                                       | 手动迭代                 |
| `cf_worker_trigger.js` | Cloudflare Worker 定时触发器（部署在 CF，本文件为源码存档）  | 手动迭代                 |
...
| 项       | 值                                                           |
| -------- | ------------------------------------------------------------ |
| Worker   | `ds-scan-trigger`（Cloudflare，源码存档 `cf_worker_trigger.js`） |
