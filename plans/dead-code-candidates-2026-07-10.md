...
> # ⚠️ 本清单已过期 6/6 中的 5 条（2026-07-27 复验）
>
> **别照着这份清单去找代码，多数已经不存在了。** 复验结果：
>
> | 原候选 | 2026-07-27 实际状态 |
> |---|---|
> | 1. `automation/gemini_reliability_check.py` | ✅ **仍是真死代码**，全仓库无任何引用 |
> | 2. `removeCardWithAnimation()` | ❌ 已删除，全仓库零命中 |
> | 3. `.holding-card.exit` | ❌ 已删除 |
> | 4. `.btn-danger` | ❌ 已删除 |
> | 5. `.card-actions` | ❌ 已删除 |
> | 6. `.card-secondary-actions` | ❌ 已删除 |
>
> 另需更正一处易被误读的对照：`ai_review.py` 与 `gemini_review.py` **都在使用中**
> （`automation/generate_dashboard.py:40-41` 有 import），不属于任何死代码候选。
>
> **这份清单本身就是"归档层会烂"的活样本**：2.5 周内 83% 的内容失效，而它作为一份
> 静态文档没有任何机制会提醒读者这一点。处置建议见下方"2026-07-27 处置意见"。
>
> ---
>
> **原始性质说明（保留）：事实性扫描结果，不含处理建议。** 只列"扫了什么、用了什么方法、发现了什么"，不删任何代码，给 Opus 审计当现成素材。是否删、怎么删由 Opus 出规格 + 大亨拍板。
>
...
## 2026-07-27 处置意见（待大亨拍板）

**对唯一存活的候选**：`automation/gemini_reliability_check.py` 建议删除。依据——
Gemini 转正评估已于 2026-07-10 放弃（见 `gemini-api-shadow-archive.md`），
`scan.yml` 不调用它，无任何 `.py` import 它，且当前 `AI_PROVIDER` 默认 `none`，
连日常 AI 审计本身都未启用。删除不影响任何运行链路，代码在 git 历史里可随时取回。
**未擅自执行**：本仓库约定代码删除由大亨拍板。

**对本文件自身**：建议连同 `plans/` 目录一并处置，理由见 `CLAUDE.md`
「归档层的处置」一节。核心是这份清单已自证——一份没人维护的静态清单，
2.5 周就能烂掉 83%，而它烂掉的方式是**看不出来**的。
