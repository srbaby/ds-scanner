...
| 版本                    | 日期       | 核心变更                                                     |
| ----------------------- | ---------- | ------------------------------------------------------------ |
| 前端合一                | 2026-06-12 | `index.html`与`dashboard.html`合并为单页：持仓管理置顶，🤖今日AI分析默认展开，📡report原文默认折叠；同步更新`X-Plan.md`执行流程描述 |
| 文档去重+方法论文档修订 | 2026-06-12 | CLAUDE.md剥离全部规则摘要，替换为速查索引；`X-Plan.md`扫描器policy分实现迁入附录A；canonical文件重命名为`X-Plan.md`（版本号移入文件内部头部） |
